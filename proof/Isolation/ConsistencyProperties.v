...
forall va : vaddr, forall level stop, Some level = getNbLevel ->
...
forall va pg pd sh1, getPd partition (memory s) = Some pd ->
...
forall va pg pd sh2, getPd partition (memory s) = Some pd ->
...
forall (partition : page), In partition (getPartitions pageRootPartition s) ->
forall pdroot, getPd partition (memory s) = Some pdroot ->
forall structroot, nextEntryIsPP partition idxroot structroot s ->
forall nbL stop, Some nbL = getNbLevel ->
forall indirection1 va b, getIndirection pdroot va nbL stop s = Some indirection1 ->
(Nat.eqb pageDefault indirection1) = b ->
(exists indirection2, getIndirection structroot va nbL stop s = Some indirection2 /\
...
forall (partition : page), In partition (getPartitions pageRootPartition s) ->
forall (idxroot : index), (idxroot = idxPageDir \/
                           idxroot = idxShadow1 \/
                           idxroot = idxShadow2 \/
                           idxroot = idxShadow3 \/
...
forall partition va pd sh1 page, In partition (getPartitions pageRootPartition s) ->
...
forall partition va pd accessiblePage, In partition (getPartitions pageRootPartition s) ->
getPd partition (memory s) = Some pd ->
...
forall ancestor partition, In partition (getPartitions pageRootPartition s) ->
...
forall partition1 partition2, In partition1 (getPartitions pageRootPartition s) ->
...
forall partition parent : page, In partition (getPartitions pageRootPartition s) ->
...
forall partition parent : page, In parent (getPartitions pageRootPartition s) ->
In partition (getChildren parent s) ->
getParent partition (memory s) = Some parent.
...
forall parent va pdParent pageParent, In parent (getPartitions pageRootPartition s) ->
getPd parent (memory s) = Some pdParent ->
~isDerived parent va s ->
getMappedPage pdParent s va = SomePage pageParent ->
forall child pdChild vaInChild pageChild, In child (getChildren parent s) ->
...
forall partition va pd sh1, In partition (getPartitions pageRootPartition s) ->
getPd partition (memory s) = Some pd ->
getFstShadow partition (memory s) = Some sh1 ->
...
forall part fstLL LLtable, In part (getPartitions pageRootPartition s) -> 
...
forall part fstLL LLtable maxidx, In part (getPartitions pageRootPartition s) ->
nextEntryIsPP part idxShadow3 fstLL s ->
In LLtable (getLLPages part s nbPage) ->
getMaxIndex = Some maxidx ->
...
nextEntryIsPP part idxShadow3 fstLL s ->
In LLtable (getLLPages part s (nbPage+1)) ->
exists FFI nextFFI, readIndex LLtable (CIndex 0) (memory s) = Some FFI
/\ isVA LLtable FFI s
/\ FFI < tableSize - 1
/\ Index.succ FFI = Some nextFFI
/\ isI LLtable nextFFI s.

Definition LLconfiguration5 s:=
forall part fstLL, In part (getPartitions pageRootPartition s) ->
nextEntryIsPP part idxShadow3 fstLL s ->
NoDup (getLLPages fstLL s (nbPage + 1)).

Definition LLconfiguration5' s :=
forall partition LL LLtable FFI nextFFI, In partition (getPartitions pageRootPartition s) ->
...
Index.succ FFI = Some nextFFI ->
getMaxIndex <> Some nextFFI /\ nextFFI <> CIndex 1.
